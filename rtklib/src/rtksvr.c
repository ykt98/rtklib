...
/* 解码接收器原始/rtcm数据 decode receiver raw/rtcm data ---------------------------------------------*/
...
#endif
{
    ...
    for (cycle=0;svr->state;cycle++) {
        ...
        for (i=0;i<3;i++) {
            ...
            /* 从输入流中读取接收器原始/rtcm数据 read receiver raw/rtcm data from input stream */
                       ...
                       solopt_t *solopt, stream_t *moni, char *errmsg)
{
    ...
    for (i=0;i<3;i++) { /* input/log streams */
        ...
        /* 初始化接收器原始和rtcm控制 initialize receiver raw and rtcm control */
        ...
        /* 连接dgps校正 connect dgps corrections */
        svr->rtcm[i].dgps=svr->nav.dgps;
    }
    for (i=0;i<2;i++) { /* 输出peek缓冲器 output peek buffer */
    ...
    for (i=0;i<8;i++) {
        ...
        /* 设置rtcm和raw的初始时间 set initial time for rtcm and raw */
        if (i<3) {
    ...
    /* 同步输入流 sync input streams */
    strsync(svr->stream,svr->stream+1);
    strsync(svr->stream,svr->stream+2);
    
    /* 将开始命令写入输入流 write start commands to input streams */
    for (i=0;i<3;i++) {
        if (!cmds[i]) continue;
    ...
    /* 将解决方案标题写入解决方案流 write solution header to solution streams */
    for (i=3;i<5;i++) {
        writesolhead(svr->stream+i,svr->solopt+i-3);
    }
    /* 创建rtk服务器线程 create rtk server thread */
#ifdef WIN32
