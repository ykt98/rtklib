...
#define AS2R        (D2R/3600.0)        /*弧度的弧度秒 arc sec to radian */

#define OMGE        7.2921151467E-5     /* 地球角速度 earth angular velocity (IS-GPS) (rad/s) */

#define RE_WGS84    6378137.0           /* 地球半长轴 earth semimajor axis (WGS84) (m) */
#define FE_WGS84    (1.0/298.257223563) /* 地球扁率 earth flattening (WGS84) */
...
#define MAXSAT      (NSATGPS+NSATGLO+NSATGAL+NSATQZS+NSATCMP+NSATIRN+NSATSBS+NSATLEO)
                                        /* 最大卫星数量max satellite number (1 to MAXSAT) */
#define MAXSTA      255
...
#else
#define DTTOL       0.025               /*时差容限（s）  tolerance of time difference (s) */
#endif
#define MAXDTOE     7200.0              /* 与全球定位系统前束的最大时差 max time difference to GPS Toe (s) */
...
#define EPHOPT_BRDC 0                   /* 星历选项:广播星历 ephemeris option: broadcast ephemeris */
...
#define ARMODE_WLNL 4                   /* AR mode:宽车道/窄车道 wide lane/narrow lane */
#define ARMODE_TCAR 5                   /* AR mode:三载波ar triple carrier ar */
...
#define STR_SERIAL   1                  /* 流类型:串行 stream type: serial */
...
typedef struct {        /* time struct */
    time_t time;        /* 用标准时间表示的时间 time (s) expressed by standard time_t */
    double sec;         /* fraction of second under 1 s */
} gtime_t;

typedef struct {        /* observation data record */
    gtime_t time;       /* 接收器采样时间 receiver sampling time (GPST) */
    unsigned char sat,rcv; /* satellite/接收方编号 receiver number */
    unsigned char SNR [NFREQ+NEXOBS]; /* signal strength (0.25 dBHz) */
    unsigned char LLI [NFREQ+NEXOBS]; /* loss of lock indicator */
    unsigned char code[NFREQ+NEXOBS]; /* 代码指示器 code indicator (CODE_???) */
    double L[NFREQ+NEXOBS]; /* 观测数据载波相位(周期) observation data carrier-phase (cycle) */
...
typedef struct {        /* antenna parameter type */
    ...
    double off[NFREQ][ 3]; /* 相位中心偏移e / n / u或x / y / z（m）  phase center offset e/n/u or x/y/z (m) */
    double var[NFREQ][19]; /* 相位中心变化（m） phase center variation (m) */
                        /* el=90,85,...,0 or nadir=0,1,2,3,... (deg) */
} pcv_t;

typedef struct {        /* 天线参数类型 antenna parameters type */
...
typedef struct {        /* GPS/QZS/GAL broadcast ephemeris type */
    ...
    double f0,f1,f2;    /* SV时钟参数 SV clock parameters (af0,af1,af2) */
    double tgd[4];      /* 群延迟参数 group delay parameters */
                        /* GPS/QZS:tgd[0]=TGD */
...
typedef struct {        /* GLONASS broadcast ephemeris type */
    ...
    double vel[3];      /*卫星速度(ecef)  satellite velocity (ecef) (m/s) */
...
typedef struct {        /* precise ephemeris type */
    ...
    int index;          /*多个文件的星历索引  ephemeris index for multiple files */
    double pos[MAXSAT][4]; /* satellite position/clock (ecef) (m|s) */
    float  std[MAXSAT][4]; /* 卫星位置/时钟标准（m | s） satellite position/clock std (m|s) */
    double vel[MAXSAT][4]; /* 卫星速度/船心率（m / s | s / s satellite velocity/clk-rate (m/s|s/s) */
    float  vst[MAXSAT][4]; /* 卫星速度/ clk-rate std（m / s | s / s） satellite velocity/clk-rate std (m/s|s/s) */
...
typedef struct {        /* precise clock type */
    ...
    double clk[MAXSAT][1]; /*卫星时钟 satellite clock (s) */
...
typedef struct {        /* TEC grid type */
    ...
    int ndata[3];       /* TEC网格数据大小 TEC grid data size {nlat,nlon,nhgt} */
    double rb;          /*地球半径  earth radius (km) */
    double lats[3];     /* latitude start/interval (deg) */
    double lons[3];     /* longitude start/interval (deg) */
    double hgts[3];     /* 高度起点/间隔(km) heights start/interval (km) */
...
typedef struct {        /* navigation data type */
    int n,nmax;         /*广播星历数量  number of broadcast ephemeris */
    int ng,ngmax;       /* number of glonass ephemeris */
    int ns,nsmax;       /* number of sbas ephemeris */
    int ne,nemax;       /* 精密星历数 number of precise ephemeris */
    int nc,ncmax;       /* number of precise clock */
    int na,namax;       /* number of almanac data */
    int nt,ntmax;       /* tec网格数据的数量 number of tec grid data */
    int nf,nfmax;       /* number of satellite fcb data */
    eph_t *eph;         /* GPS/QZS/GAL 星历表 ephemeris */
    geph_t *geph;       /* GLONASS ephemeris */
    seph_t *seph;       /* SBAS ephemeris */
    peph_t *peph;       /* precise ephemeris */
    pclk_t *pclk;       /*精密时钟 precise clock */
    alm_t *alm;         /* almanac data */
    tec_t *tec;         /* tec grid data */
    fcbd_t *fcb;        /* satellite fcb data */
    erp_t  erp;         /*地球自转参数 earth rotation parameters */
    ...
    double ion_gps[8];  /*全球定位系统电离层模型参数 GPS iono model parameters {a0,a1,a2,a3,b0,b1,b2,b3} */
    double ion_gal[4];  /* Galileo iono model parameters {ai0,ai1,ai2,0} */
    double ion_qzs[8];  /* QZSS iono model parameters {a0,a1,a2,a3,b0,b1,b2,b3} */
    double ion_cmp[8];  /* BeiDou iono model parameters {a0,a1,a2,a3,b0,b1,b2,b3} */
    double ion_irn[8];  /* IRNSS iono model parameters {a0,a1,a2,a3,b0,b1,b2,b3} */
    int leaps;          /* leap seconds (s) */
    double lam[MAXSAT][NFREQ]; /* 载波波长（m） carrier wave lengths (m) */
    double cbias[MAXSAT][3]; /* satellite dcb (0:p1-p2,1:p1-c1,2:p2-c2) (m) */
    double rbias[MAXRCV][2][3]; /* receiver dcb (0:p1-p2,1:p1-c1,2:p2-c2) (m) */
    double wlbias[MAXSAT];   /* 宽车道偏差 wide-lane bias (cycle) */
...
typedef struct {        /* solution type */
    gtime_t time;       /* time (GPST) */
    double rr[6];       /* 位置/速度（m | m / s） position/velocity (m|m/s) */
                        /* {x,y,z,vx,vy,vz} or {e,n,u,ve,vn,vu} */
    float  qr[6];       /* 位置方差/协方差(m^2) position variance/covariance (m^2) */
                        /* {c_xx,c_yy,c_zz,c_xy,c_yz,c_zx} or */
                        /* {c_ee,c_nn,c_uu,c_en,c_nu,c_ue} */
    float  qv[6];       /* velocity variance/covariance (m^2/s^2) */
    double dtr[6];      /* receiver clock bias to time systems (s) */
    unsigned char type; /* type (0:xyz-ecef,1:enu-baseline) */
    unsigned char stat; /* 解决方案状态 solution status (SOLQ_???) */
...
typedef struct {        /* SNR mask type */
    int ena[2];         /* 启用标志 enable flag {rover,base} */
    double mask[NFREQ][9]; /* mask (dBHz) at 5,10,...85 deg */
} snrmask_t;

typedef struct {        /* 处理选项类型 processing options type */
    int mode;           /* positioning mode (PMODE_???) */
    int soltype;        /* 解决方案类型（0：前进，1：后退，2：组合） solution type (0:forward,1:backward,2:combined) */
    int nf;             /*频率数量 number of frequencies (1:L1,2:L1+L2,3:L1+L2+L5) */
    int navsys;         /* navigation system */
    double elmin;       /* 高程遮罩角度 elevation mask angle (rad) */
    ...
    int ionoopt;        /* 电离层选项 ionosphere option (IONOOPT_???) */
    int tropopt;        /*对流层期权 troposphere option (TROPOPT_???) */
    int dynamics;       /* 动力学模型 dynamics model (0:none,1:velociy,2:accel) */
    ...
    int refpos;         /* 相对模式的基本位置 base position for relative mode */
                        /* (0:pos in prcopt,  1:平均单次average of single pos, */
                        /*  2:从文件读取 read from file, 3:rinex头 rinex header, 4:rtcm pos) */
    double eratio[NFREQ]; /* code/phase error ratio */
    double err[5];      /*测量误差因素 measurement error factor */
                        /* [0]:reserved */
                        /* [1-3]:相位误差系数a/b/c error factor a/b/c of phase (m) */
                        /* [4]:多普勒频率 doppler frequency (hz) */
    double std[3];      /*初始状态标准  initial-state std [0]bias,[1]iono [2]trop */
    double prn[6];      /*过程噪声标准 process-noise std [0]bias,[1]iono [2]trop [3]acch [4]accv [5] pos */
    double sclkstab;    /* satellite clock stability (sec/sec) */
    double thresar[8];  /*AR验证阈值 AR validation threshold */
    double elmaskar;    /* elevation mask of AR for rising satellite (deg) */
    double elmaskhold;  /* elevation mask to hold ambiguity (deg) */
    double thresslip;   /* slip threshold of geometry-free phase (m) */
    double maxtdiff;    /* 最大时间差（秒） max difference of time (sec) */
    double maxinno;     /* reject threshold of innovation (m) */
    double maxgdop;     /* reject threshold of gdop */
    double baseline[2]; /* baseline length constraint {const,sigma} (m) */
    double ru[3];       /* 固定模式下的流动站位置 rover position for fixed mode {x,y,z} (ecef) (m) */
    double rb[3];       /* 相对模式的基站位置 base position for relative mode {x,y,z} (ecef) (m) */
    ...
    int  outsingle;     /*通过dgps / float / fix / ppp中断输出单个 output single by dgps/float/fix/ppp outage */
    char rnxopt[2][256]; /* rinex options {rover,base} */
    int  posopt[6];     /* positioning options */
    int  syncsol;       /* solution sync mode (0:off,1:on) */
    double odisp[2][6*11]; /*海洋潮汐载荷参数 ocean tide loading parameters {rov,base} */
...
typedef struct {        /* solution options type */
    ...
    int solstatic;      /*静态模式的解决方案（0：全部，1：单个） solution of static mode (0:all,1:single) */
...
typedef struct {        /* file options type */
    char satantp[MAXSTRPATH]; /* 卫星天线参数文件 satellite antenna parameters file */
    char rcvantp[MAXSTRPATH]; /* receiver antenna parameters file */
    char stapos [MAXSTRPATH]; /* 站位置文件station positions file */
    char geoid  [MAXSTRPATH]; /* 外部大地水准面数据文件 external geoid data file */
    char iono   [MAXSTRPATH]; /* 电离层数据文件 ionosphere data file */
    char dcb    [MAXSTRPATH]; /* dcb data file */
    char eop    [MAXSTRPATH]; /* eop data file */
    char blq    [MAXSTRPATH]; /* 海洋潮汐加载blq文件 ocean tide loading blq file */
    char tempdir[MAXSTRPATH]; /* ftp/http临时目录 ftp/http temporaly directory */
...
typedef struct {        /* RINEX options type */
    ...
    double tint;        /* time interval (s)时间间隔 */
    double ttol;        /* time tolerance (s) */
    double tunit;       /* 多会话的时间单位 time unit for multiple-session (s) */
    double rnxver;      /* RINEX version */
    int navsys;         /* navigation system导航系统 */
    int obstype;        /* observation type */
    int freqtype;       /* frequency type 频率类型*/
    ...
    int scanobs;        /* scan obs types扫描obs类型 */
    int outiono;        /* output iono correction输出电离层校正 */
    int outtime;        /* output time system correction输出时间校正系统 */
    ...
    gtime_t trtcm;      /* approx log start time for rtcm RTCM大约日志开始时间*/
    char tobs[7][MAXOBSTYPE][4]; /* obs types {GPS,GLO,GAL,QZS,SBS,CMP,IRN} */
    int nobs[7];        /* number of obs types {GPS,GLO,GAL,QZS,SBS,CMP,IRN} */
} rnxopt_t;

typedef struct {        /*卫星状态类型 satellite status type */
    unsigned char sys;  /* navigation system */
    unsigned char vs;   /* valid satellite flag single */
    double azel[2];     /* azimuth/elevation angles {az,el} (rad) */
    double resp[NFREQ]; /* 伪距残差(m residuals of pseudorange (m) */
    double resc[NFREQ]; /* 载波相位残差(m) residuals of carrier-phase (m) */
    unsigned char vsat[NFREQ]; /* valid satellite flag */
    unsigned char snr [NFREQ]; /* signal strength (0.25 dBHz) */
    unsigned char fix [NFREQ]; /* 模糊修复标志(1:固定，2:浮动，3:保持) ambiguity fix flag (1:fix,2:float,3:hold) */
    unsigned char slip[NFREQ]; /* cycle-slip flag */
    unsigned char half[NFREQ]; /* half-cycle valid flag */
    int lock [NFREQ];   /*相位锁定计数器 lock counter of phase */
    unsigned int outc [NFREQ]; /*相位obs中断计数器 obs outage counter of phase */
...
typedef struct {        /* RTK control/result type */
    ...
    int nx,na;          /* 浮点状态数 number of float states/fixed states */
    double tt;          /*当前时间与先前时间之间的时差  time difference between current and previous (s) */
    double *x, *P;      /* 浮动状态 float states and their covariance */
    double *xa,*Pa;     /* fixed states and their covariance */
    int nfix;           /* 模糊度的连续修正数 number of continuous fixes of ambiguity */
    ambc_t ambc[MAXSAT]; /* ambibuity control */
    ssat_t ssat[MAXSAT]; /* 卫星状态 satellite status */
...
typedef struct {        /* 流转换器类型 stream converter type */
...
typedef struct {        /* 流服务器类型 stream server type */
...
typedef struct {        /* RTK server type */
    ...
    int nb [3];         /*输入缓冲区{rov，base}中的字节数 bytes in input buffers {rov,base} */
    ...
    int prcout;         /*缺失观察数据计数 missing observation data count */
...
EXPORT int  readerp(const char *file, erp_t *
);
