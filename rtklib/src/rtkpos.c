...
#define GAP_RESION  120      /* 重置电离层参数的间隙(纪元) gap to reset ionosphere parameters (epochs) */
...
#define TTOL_MOVEB  (1.0+2*DTTOL)
                             /* 移动基准的时间同步容限 time sync tolerance for moving-baseline (s) */
...
extern int rtkoutstat(rtk_t *rtk, char *buff)
{
    ...
    /*接收器速度和加速度 receiver velocity and acceleration */
...
/* 输出解决方案状态 output solution status ----------------------------------------------------*/
...
static void udion(rtk_t *rtk, double tt, double bl, const int *sat, int ns)
{
    ...
    for (i=0;i<ns;i++) {
        ...
        if (rtk->x[j]==0.0) {
            ...
            /* 过程噪声的高度相关因子 elevation dependent factor of process noise */
...
static void detslp_ll(rtk_t *rtk, const obsd_t *obs, int i, int rcv)
{
    ...
    for (f=0;f<rtk->opt.nf;f++) {
        ...
        /*通过LLI的循环滑移标志检测滑移 detect slip by cycle slip flag in LLI */
        ...
        /*在LLI通过奇偶校验未知标志转换检测滑动 detect slip by parity unknown flag transition in LLI */
        if (((LLI&2)&&!(obs[i].LLI[f]&2))||(!(LLI&2)&&(obs[i].LLI[f]&2))) {
            errmsg(rtk,"检测到滑动半周slip detected half-cyc (sat=%2d rcv=%d F=%d LLI=%x->%x)\n",
                   ...
                   const int *iu, const int *ir, int ns, const nav_t *nav)
{
    ...
    for (f=0;f<nf;f++) {
        /*如果瞬时增强现实或光突发交换中断计数器过期，重置相位偏置 reset phase-bias if instantaneous AR or expire obs outage counter */
        for (i=1;i<=MAXSAT;i++) {
        ...
        /*如果检测到周跳，复位相位偏置 reset phase-bias if detecting cycle slip */
        for (i=0;i<ns;i++) {
        ...
        /* 用相位码估计近似相位偏差 estimate approximate phase-bias by phase - code */
        ...
        /*校正相位偏移以确保相位代码一致性 correct phase-bias offset to enssure phase-code coherency */
        if (j>0) {
            for (i=1;i<=MAXSAT;i++) {
                if (rtk->x[IB(i,f,&rtk->opt)]!=0.0) rtk->x[IB(i,f,&rtk->opt)]+=offset/j;
            }
        }
        /*设置相位偏置的初始状态 set initial states of phase-bias */
        for (i=0;i<ns;i++) {
                    ...
                    const int *iu, const int *ir, int ns, const nav_t *nav)
{
    ...
    /*接收器h/w偏差的时间更新 temporal update of eceiver h/w bias */
                 ...
                 int index, double *y, double *e, double *azel)
{
    ...
    for (i=0;i<n;i++) {
        /*计算几何距离和方位角/仰角 compute geometric-range and azimuth/elevation angle */
        ...
        /*对流层延迟模型(流体静力学) troposphere delay model (hydrostatic) */
        zhd=tropmodel(obs[0].time,pos,zazel,0.0);
        r+=tropmapf(obs[i].time,pos,azel+i*2,NULL)*zhd;
        
        /* 接收器天线相位中心校正 receiver antenna phase center correction */
        antmodel(opt->pcvr+index,opt->antdel[index],azel+i*2,opt->posopt[1],
                 dant);
        
        /*卫星的无差相位/编码残差 undifferenced phase/code residual for satellite */
...
/* 测试有效的观察数据 test valid observation data -----------------------------------------------*/
                 ...
                 double *H, double *R, int *vflg)
{
    ...
    /*电离层和对流层延迟计算因子 compute factors of ionospheric and tropospheric delay */
    for (i=0;i<ns;i++) {
    ...
    for (m = 0; m < 5; m++) /* m=0:gps/sbs,1:glo,2:gal,3:bds,4:qzs */
    {
        for (f = opt->mode > PMODE_DGPS ? 0 : nf; f < nf * 2; f++) {
            ...
            /*搜索最高的参考卫星 search reference satellite with highest elevation */
            for (i = -1, j = 0; j < ns; j++) {
            ...
            for (j = 0; j < ns; j++) {
                ...
                /*流动站位置的偏导数  partial derivatives by rover position */
                if (H) {
                    for (k = 0; k < 3; k++) {
                ...
                /* 双差分对流层延迟项 double-differenced tropospheric delay term */
...
#if 0 /* 参考卫星的残差(2.4.2 p11) residuals referenced to reference satellite (2.4.2 p11) */
    ...
    /* 移动基线的基线长度约束 baseline length constraint for moving baseline */
...
/* 单到双差分转换矩阵（D'） single to double-difference transformation matrix (D') --------------------*/
...
static int resamb_LAMBDA(rtk_t *rtk, double *bias, double *xa)
{
    ...
    /* 相位偏差协方差（Qb）和实参来偏差协方差（Qab） phase-bias covariance (Qb) and real-parameters to bias covariance (Qab) */
    ...
    /*lambda / mlambda整数最小二乘估计 lambda/mlambda integer least-square estimation */
                  ...
                  const nav_t *nav)
{
    ...
    /*卫星位置/时钟 satellite positions/clocks */
    satposs(time,obs,n,nav,opt->sateph,rs,dts,var,svh);
    
    /*基站的无差异残差 undifferenced residuals for base station */
    ...
    /*残差的时间插值(用于后处理) time-interpolation of residuals (for post-processing) */
    if (opt->intpref) {
        dt=intpres(time,obs+nu,nr,nav,rtk,y+nu*nf*2);
    }
    /*选择流动站和基站之间的公共卫星 select common satellites between rover and base-station */
    ...
    /*状态的时间更新 temporal update of states */
    ...
    if (stat!=SOLQ_NONE&&zdres(0,obs,nu,rs,dts,var,svh,nav,xp,opt,0,y,e,azel)) {
        ...
        /*浮动解的后拟合残差 post-fit residuals for float solution */
        nv=ddres(rtk,nav,dt,xp,Pp,sat,y,e,azel,iu,ir,ns,v,NULL,R,vflg);
        
        /* 验证validation of float solution */
        if (valpos(rtk,v,R,vflg,nv,4.0)) {
            
            /*更新状态和协方差矩阵 update state and covariance matrix */
            matcpy(rtk->x,xp,rtk->nx,1);
            matcpy(rtk->P,Pp,rtk->nx,rtk->nx);
            
            /*更新模糊控制结构 update ambiguity control struct */
            rtk->sol.ns=0;
            ...
            /*缺乏有效的卫星 lack of valid satellites */
            if (rtk->sol.ns<4) stat=SOLQ_NONE;
        }
        else stat=SOLQ_NONE;
    }
    /*用WL-NL法求解整周模糊度 resolve integer ambiguity by WL-NL */
    ...
    /*TCAR解决整周模糊度 resolve integer ambiguity by TCAR */
    ...
    /*通过LAMBDA解决整数模糊 resolve integer ambiguity by LAMBDA */
        ...
        if (zdres(0,obs,nu,rs,dts,var,svh,nav,xa,opt,0,y,e,azel)) {
            ...
            if (valpos(rtk,v,R,vflg,nv,4.0)) {
                ...
                /* 保持整数模糊度 hold integer ambiguity */
...
/* 初始化rtk控件 initialize rtk control ------------------------------------------------------
...
*            rtk->x[]       IO  前置过滤器和后置过滤器的浮动状态 float states pre-filter and post-filter
...
* 在调用函数之前，基站位置rtk-> sol.rb []应该正确设置相对模式（移动基线除外）
*-----------------------------------------------------------------------------*/
extern int rtkpos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    /*设置基站位置  set base staion position */
    if (opt->refpos<=POSOPT_RINEX&&opt->mode!=PMODE_SINGLE&& 
        opt->mode!=PMODE_MOVEB) {
        for (i=0;i<6;i++) rtk->rb[i]=i<3?opt->rb[i]:0.0;
    }
    /* 计算流动站/基站观测值 count rover/base station observations */
    for (nu=0;nu   <n&&obs[nu   ].rcv==1;nu++) ;
    for (nr=0;nu+nr<n&&obs[nu+nr].rcv==2;nr++) ;
    
    time=rtk->sol.time; /* 上一个历元 previous epoch */
    
    /*流动站位置 单点定位  rover position by single point positioning */
    ...
    /* 抑制单个解决方案的输出 suppress output of single solution */
    if (!opt->outsingle) {
        rtk->sol.stat=SOLQ_NONE;
    }
    /* 精密单点定位precise point positioning */
    if (opt->mode>=PMODE_PPP_KINEMA) {
        pppos(rtk,obs,nu,nav);
        outsolstat(rtk);
        return 1;
    }
    /* 检查基站数据数量和差分年龄 check number of data of base station and age of differential */
    if (nr==0) {
        errmsg(rtk,"no base station observation data for rtk\n");
        outsolstat(rtk);
        return 1;
    }
    if (opt->mode==PMODE_MOVEB) { /* 移动基线 moving baseline */
        
        /* 估计基站的位置/速度 estimate position/velocity of base station */
        ...
        /* 基站时间同步位置 time-synchronized position of base station */
