...
*           2013/03/11  1.13 add function reading otl and 
data
...
static int iobsu =0;            /*当前流动站观测数据指标 current rover observation data index */
static int iobsr =0;            /* 当前参考观测数据索引 current reference observation data index */
static int isbs  =0;            /* current sbas message index */
static int ilex  =0;            /* current lex message index */
static int revs  =0;            /*分析方向（0：向前，1：向后）  analysis direction (0:forward,1:backward) */
static int aborts=0;            /*中止状态 abort status */
...
static char proc_base[64]="";   /* 当前处理基站 base station for current processing */
static char rtcm_file[1024]=""; /* rtcm data file */
static char rtcm_path[1024]=""; /* rtcm data path */
static rtcm_t rtcm;             /* rtcm control struct */
static FILE *fp_rtcm=NULL;      /* rtcm数据文件指针 rtcm data file pointer */
                    ...
                    int mode)
{
    ...
    solstatic=sopt->solstatic&&
              (popt->mode==PMODE_STATIC||popt->mode==PMODE_PPP_STATIC);//output 中的solution mode
    
    rtkinit(&rtk,popt);//初始化
    rtcm_path[0]='\0';
    
    while ((nobs=inputobs(obs,rtk.sol.stat,popt))>=0) {
        
        /* 排除卫星 exclude satellites */
        for (i=n=0;i<nobs;i++) {
            if ((satsys(obs[i].sat,NULL)&popt->navsys)&&
                popt->exsats[obs[i].sat-1]!=1) obs[n++]=obs[i];//(1:excluded,2:included)
        }
        if (n<=0) continue;
        
        /*载波相位偏差校正  carrier-phase bias correction */
        if (navs.nf>0) {
                        ...
                        nav_t *nav, sbs_t *sbs, lex_t *lex)
{
    ...
    /* 读取精确的时钟文件 read precise clock files */
    for (i=0;i<n;i++) {
        if (strstr(infile[i],"%r")||strstr(infile[i],"%b")) continue;
...
/*天线相位中心位置  antenna phase center position ---------------------------------------------*/
                   ...
                   const filopt_t *fopt, nav_t *nav, pcvs_t *pcvs, pcvs_t *pcvr)
{
    ...
    /* 读取卫星天线参数 read satellite antenna parameters */
    ...
    /* 如果不存在L5偏移，则使用卫星L2偏移 use satellite L2 offset if L5 offset does not exists */
    for (i=0;i<pcvs->n;i++) {
        if (norm(pcvs->pcv[i].off[2],3)>0.0) continue;
        matcpy(pcvs->pcv[i].off[2],pcvs->pcv[i].off[1], 3,1);//复制矩阵
                   ...
                   char **infile, const int *index, int n, char *outfile)
{
    ...
    /*读取电离层数据文件  read ionosphere data file */
    ...
    if (*fopt->dcb) {
        ...
        readdcb(path,&navs,stas);//读取差分代码偏置（dcb）参数
    }
    /*设置天线参数 set antenna paramters */
    if (popt_.mode!=PMODE_SINGLE) {
        setpcv(obss.n>0?obss.data[0].time:timeget(),&popt_,&navs,&pcvss,&pcvsr,
               stas);
    }
    /*读取海潮加载参数  read ocean tide loading parameters */
    if (popt_.mode>PMODE_SINGLE&&*fopt->blq) {
        readotl(&popt_,fopt->blq,stas);
    }
    /* 流动站/参考 固定位置 rover/reference fixed position */
    ...
    /* 打开解决方案统计 open solution statistics */
...
/* 为每个基站执行处理会话 execute processing session for each base station --------------------------*/
                     ...
                     const char *rov, const char *base)
{
    ...
    /*读取精密星历表和sbas数据 read prec ephemeris and sbas data */
    readpreceph(infile,n,popt,&navs,&sbss,&lexs);
    
    for (i=0;i<n;i++) if (strstr(infile[i],"%b")) break;
    
    if (i<n) { /* 包括基站关键字 include base station keywords */
        ...
        for (p=base_;;p=q+1) { /* for each base station */
            if ((q=strchr(p,' '))) *q='\0';//strchr函数功能为在一个串中查找给定字符的第一个匹配之处
                   ...
                   const char *rov, const char *base)
{
    ...
    /* 打开处理会话 open processing session */
    ...
    if (ts.time!=0&&te.time!=0&&tu>=0.0) {
        ...
        for (i=0;;i++) { /* for each periods */
            ...
            for (j=k=nf=0;j<n;j++) {
                ...
                ext=strrchr(infile[j],'.');//返回.后面的内容
                
                if (ext&&(!strcmp(ext,".rtcm3")||!strcmp(ext,".RTCM3"))) {//用于比较两个字符串并根据比较结果返回整数。
                    strcpy(ifile[nf++],infile[j]);
                }
                else {
                    /*包括次日精确的星历表或广播星历 include next day precise ephemeris or rinex brdc nav */
                    ttte=tte;
            ...
            /* 执行处理会话 execute processing session */
