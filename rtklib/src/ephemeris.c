...
#define MU_GPS   3.9860050E14     /*重力常数 gravitational constant         ref [1] */
#define MU_GLO   3.9860044E14     /* gravitational constant         ref [2] */
#define MU_GAL   3.986004418E14   /*地球引力常数 earth gravitational constant   ref [7] */
#define MU_CMP   3.986004418E14   /* earth gravitational constant   ref [9] */
#define J2_GLO   1.0826257E-3     /* 2nd zonal harmonic of geopot   ref [2] */

#define OMGE_GLO 7.292115E-5      /*地球角速度 earth angular velocity (rad/s) ref [2] */
...
#define TSTEP    60.0             /* 积分步长glonass星历表 integration step glonass ephemeris (s) */
...
#define STD_BRDCCLK 30.0          /* 广播时钟误差(m) error of broadcast clock (m) */
#define STD_GAL_NAPA 500.0        /* error of galileo ephemeris for NAPA (m) */

#define MAX_ITER_KEPLER 30        /* 开普勒最大迭代次数 max number of iteration of Kelpler */

/* 星历表选择 ephemeris selections ------------------------------------------------------*/
static int eph_sel[]={ /* GPS,GLO,GAL,QZS,BDS,SBS */
    0,0,1,0,0,0
};

/* 由ura星历表引起的方差variance by ura ephemeris -------------------------------------------------*/
...
/* 历书到卫星位置和时钟偏差 almanac to satellite position and clock bias --------------------------------
* 用历书计算卫星位置和时钟偏差 compute satellite position and clock bias with almanac (gps, galileo, qzss)
...
/* 广播星历到卫星时钟偏差 broadcast ephemeris to satellite clock bias ---------------------------------
* 用广播星历计算卫星时钟偏差 compute satellite clock bias with broadcast ephemeris (gps, galileo, qzss)
...
/* 广播星历到卫星位置和时钟偏差 broadcast ephemeris to satellite position and clock bias --------------------
* 用广播星历计算卫星位置和时钟偏差 compute satellite position and clock bias with broadcast ephemeris (gps,
* galileo, qzss)
* args   : gtime_t time     I   time (gpst)
*          eph_t *eph       I   broadcast ephemeris
*          double *rs       O   satellite position (ecef) {x,y,z} (m)
*          double *dts      O   卫星时钟偏差 satellite clock bias (s)
                    ...
                    double *var)
{
    ...
    M=eph->M0+(sqrt(mu/(eph->A*eph->A*eph->A))+eph->deln)*tk;//计算平近点角
    ...
    u=atan2(sqrt(1.0-eph->e*eph->e)*sinE,cosE-eph->e)+eph->omg;          //计算升交距角
    r=eph->A*(1.0-eph->e*cosE);
    i=eph->i0+eph->idot*tk;
    sin2u=sin(2.0*u); cos2u=cos(2.0*u);
    u+=eph->cus*sin2u+eph->cuc*cos2u;
    r+=eph->crs*sin2u+eph->crc*cos2u;
    i+=eph->cis*sin2u+eph->cic*cos2u;                //计算改正后的升交距角，卫星矢距，轨道倾角
    x=r*cos(u); y=r*sin(u); cosi=cos(i);
    
    /* beidou geo satellite */
    if (sys==SYS_CMP&&(eph->flag==2||(eph->flag==0&&prn<=5))) {
        O=eph->OMG0+eph->OMGd*tk-omge*eph->toes;           //计算升交点的经度
        sinO=sin(O); cosO=cos(O);
        xg=x*cosO-y*cosi*sinO;
        yg=x*sinO+y*cosi*cosO;
        zg=y*sin(i);                         //计算地固坐标系下的坐标
    ...
    *dts-=2.0*sqrt(mu*eph->A)*eph->e*sinE/SQR(CLIGHT);
    ...
    /* 位置和时钟误差方差 position and clock error variance */
    *var=var_uraeph(sys,eph->sva);
}

//GLONASS

/* glonass轨道微分方程  orbit differential equations --------------------------------------*/
...
/* glonass通过数值积分的位置和速度  position and velocity by numerical integration --------------------*/
...
/* glonass 星历表与卫星时钟偏差ephemeris to satellite clock bias -----------------------------------
...
*用glonass星历计算卫星位置和时钟偏差 compute satellite position and clock bias with glonass ephemeris
* args   : gtime_t time     I   time (gpst)
*          geph_t *geph     I   glonass ephemeris
...
static eph_t *seleph(gtime_t time, int sat, int iode, const nav_t *nav)
{
    ...
    for (i=0;i<nav->n;i++) {
        ...
        if (t<=tmin) {j=i; tmin=t;} /* 最接近时间的星历参考时刻 toe closest to time */
    }
    if (iode>=0||j<0) {
...
/* 带有广播星历表的卫星时钟 satellite clock with broadcast ephemeris ----------------------------------*/
...
/* 通过广播星历表确定卫星位置和时钟 satellite position and clock by broadcast ephemeris -----------------------*/
...
/*带有sbas校正的卫星位置和时钟 satellite position and clock with sbas correction -------------------------*/
...
/* 带ssr校正的卫星位置和时钟 satellite position and clock with ssr correction --------------------------*/
...
/*卫星位置和时钟 satellite position and clock ------------------------------------------------
* 计算卫星位置、速度和时钟 compute satellite position, velocity and clock
* args   : gtime_t time     I   time (gpst)
*          gtime_t teph     I   time to select ephemeris (gpst)
*          int    sat       I   satellite number
*          nav_t  *nav      I   navigation data
*          int    ephopt    I   ephemeris option (EPHOPT_???)
*          double *rs       O   sat position and velocity (ecef) sat位置和速度(ecef)
...
* 计算卫星位置、速度和时钟 compute satellite positions, velocities and clocks
* args   : gtime_t teph     I   time to select ephemeris (gpst)
*          obsd_t *obs      I   observation data
*          int    n         I   number of observation data
*          nav_t  *nav      I   navigation data
*          int    ephopt    I   星历选项 ephemeris option (EPHOPT_???)
*          double *rs       O   卫星位置和速度  satellite positions and velocities (ecef)
*          double *dts      O   卫星钟 satellite clocks
*          double *var      O   卫星位置和时钟误差方差  sat position and clock error variances (m^2)
*          int    *svh      O   sat health flag (-1:correction not available)
* return : none
                    ...
                    int ephopt, double *rs, double *dts, double *var, int *svh)
{
    ...
    for (i=0;i<n&&i<2*MAXOBS;i++) {
        ...
        /* 卫星时钟传输时间 transmission time by satellite clock */
        time[i]=timeadd(obs[i].time,-pr/CLIGHT);
        
        /* 广播星历表造成卫星时钟偏差 satellite clock bias by broadcast ephemeris */
        ...
        /* 如果没有精确的时钟，请改用广播时钟 if no precise clock available, use broadcast clock instead */
        if (dts[i*2]==0.0) {
...
/*选择卫星星历 select satellite ephemeris --------------------------------------------------
* 选择卫星星历。调用satposs()，satpos()之前先调用它， select satellite ephemeris. call it before calling satpos(),satposs().
