...
const prcopt_t prcopt_default={ /* 默认处理选项 defaults processing options */
...
/*3d向量的外积 outer product of 3d vectors -------------------------------------------------
...
* args   : double *A        I   (加权)设计矩阵的转置 transpose of (weighted) design matrix (n x m)
*          double *y        I   (加权)测量值(m x 1) (weighted) measurements (m x 1)
*          int    n,m       I   number of parameters and measurements (n<=m)
*          double *x        O   估计参数 estmated parameters (n x 1)
*          double *Q        O   估计参数协方差矩阵(n x n) esimated parameters covariance matrix (n x n)
* return : status (0:ok,0>:error)
* notes  : for weighted least square, replace A and y by A*w and w*y (w=W^(1/2))
*          matirix stored by column-major order (fortran convention)
* 对于加权最小二乘法，将a和y替换为按列主顺序存储的A*w和w*y (w=W^(1/2)矩阵(fortran惯例)
*-----------------------------------------------------------------------------*/
...
*          int    i,n       I   子串的位置和宽度 substring position and width
...
/* 将日历日期/时间转换为时间 convert calendar day/time to time -------------------------------------------
...
/* 转换ecef到大地位置 transform ecef to geodetic postion ------------------------------------------
...
/*ecef到局部坐标变换矩阵 ecef to local coordinate transfromation matrix ------------------------------
...
/*将局部enu坐标协方差转换为xyz-ecef transform local enu coordinate covariance to xyz-ecef -----------------------
...
{
    static const double fc[][5]={ /*iau 1980章动系数 coefficients for iau 1980 nutation */
...
/*读取地球自转参数  read earth rotation parameters ----------------------------------------------
...
/* 排序和独特的观察数据 sort and unique observation data --------------------------------------------
* sort and unique observation data by time, rcv, sat
* args   : obs_t *obs    IO     observation data
* return : number of epochs
按时间，rcv，sat排序和唯一的观测数据
* args：obs_t * obs IO观察数据
* return：纪元数
*-----------------------------------------------------------------------------*/
...
extern int expath(const char *path, char *paths[], int nmax)
{
    ...
    closedir(dp);
#endif
    /*按字母顺序排列路径 sort paths in alphabetical order */
    for (i=0;i<n-1;i++) {
        for (j=i+1;j<n;j++) {
...
*          char   *rov      I   rover id string        ("": not replaced)
*          char   *base     I   base station id string ("": not replaced)
* return : 替换文件路径的数量 number of replaced file paths
...
*          double *e        O  视线矢量（ecef） line-of-sight vector (ecef)
...
extern double geodist(const double *rs, const double *rr, double *e)
{
    ...
    if (norm(rs,3)<RE_WGS84) return -1.0;//检查卫星到原点距离是否大于长半轴
    for (i=0;i<3;i++) e[i]=rs[i]-rr[i];//rs卫星，rr接收机
    r=norm(e,3);
    for (i=0;i<3;i++) e[i]/=r;
    return r+OMGE*(rs[0]*rr[1]-rs[1]*rr[0])/CLIGHT;
}
/* 卫星方位角/仰角 satellite azimuth/elevation angle -------------------------------------------
...
*          double elmin     I   仰角截止角(rad) elevation cutoff angle (rad)
                       ...
                       const double *azel)
{
    ...
    /* 地球中心角(半圆) earth centered angle (semi-circle) */
    psi=0.0137/(azel[1]/PI+0.11)-0.022;
    
    /*近赤道纬度/经度(半圆) subionospheric latitude/longitude (semi-circle) */
    phi=pos[0]/PI+psi*cos(azel[0]);
    if      (phi> 0.416) phi= 0.416;
    else if (phi<-0.416) phi=-0.416;
    lam=pos[1]/PI+psi*sin(azel[0])/cos(phi*PI);
    
    /* 地磁纬度(半圆)geomagnetic latitude (semi-circle) */
...
/* 电离层映射函数 ionosphere mapping function -------------------------------------------------
...
extern double tropmodel(gtime_t time, const double *pos, const double *azel,
                        double humi)//相对湿度
{
    const double temp0=15.0; /*海平面温度 temparature at sea level */
    ...
    e=6.108*humi*((17.15*temp-4684.0)/(temp-38.45));
...
static void sunmoonpos_eci(gtime_t tut, double *rsun, double *rmoon)
{
    ...
    /*天文参数 astronomical arguments */
    ast_args(t,f);
    
    /*黄道的倾角 obliquity of the ecliptic */
...
*在efef中获取太阳和月亮的位置 get sun and moon position in ecef
...
/* 共享库的虚拟应用程序功能 dummy application functions for shared library ----------------------------*/
#ifdef WIN_DLL
