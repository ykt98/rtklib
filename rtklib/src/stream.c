...
static int readfile(file_t *file, unsigned char *buff, int nmax, char *msg)
{
    ...
    if (file->fp_tag) {
        ...
        /* 寻找时间标签文件以获得下一个记号和文件位置 seek time-tag file to get next tick and file position */
...
/*解码tcp/ntrip路径 decode tcp/ntrip path (path=[user[:passwd]@]addr[:port][/mntpnt[:str]]) ---*/
...
/* 初始化流环境 initialize stream environment -----------------------------------------------
...
/*获取扩展流状态  get extended stream status ---------------------------------------------------
...
/* 将ntrip源表设置为ntrip-caster流 set ntrip source table to ntrip-caster stream -------------------------------
...
/*设置全局流选项  set global stream options ---------------------------------------------------
