                      ...
                      const double *hgts, double rb, double nexp, nav_t *nav)
{
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        else if (strstr(label,"END OF RMS MAP")==label) {//均方根图结束
            type=0;
            p=NULL;
        }
        else if (strstr(label,"EPOCH OF CURRENT MAP")==label) {//当前地图的时代
...
/*合并tec网格数据 combine tec grid data -----------------------------------------------------*/
                     ...
                     double *rms)
{
    ...
    *value=*rms=0.0;
    ...
    /* 获取网格tec数据 get gridded tec data */
    for (n=0;n<4;n++) {
...
/*tec网格数据引起的电离层延迟 ionosphere delay by tec grid data -----------------------------------------*/
static int iondelay(gtime_t time, const tec_t *tec, const double *pos,
                    const double *azel, int opt, double *delay, double *var)
{
    const double fact=40.30E16/FREQ1/FREQ1; /*tec-> L1电离层(m) tecu->L1 iono (m) */
    ...
    *delay=*var=0.0;
    ...
    for (i=0;i<tec->ndata[2];i++) { /* 对于层 for a layer */
        
        hion=tec->hgts[0]+tec->hgts[2]*i;
        
        /* 电离层穿透点位置 ionospheric pierce point position */
        fs=ionppp(pos,azel,tec->rb,hion,posp);
        
        if (opt&2) {
            /* 改进的单层映射函数(M-SLM)  参考文献[2] modified single layer mapping function (M-SLM) ref [2] */
            rp=tec->rb/(tec->rb+hion)*sin(0.9782*(PI/2.0-azel[1]));
            fs=1.0/sqrt(1.0-rp*rp);
        }
        if (opt&1) {
            /* 地球自转校正(太阳固定坐标) earth rotation correction (sun-fixed coordinate) */
            posp[1]+=2.0*PI*timediff(time,tec->time)/86400.0;
        }
        /* 插值tec网格数据 interpolate tec grid data */
                  ...
                  const double *azel, int opt, double *delay, double *var)
{
    ...
    /*tec网格数据的电离层延迟 ionospheric delay by tec grid data */
    ...
    if (stat[0]&&stat[1]) { /* 时间线性插值 linear interpolation by time */
        a=timediff(time,nav->tec[i-1].time)/tt;
        *delay=dels[0]*(1.0-a)+dels[1]*a;
        *var  =vars[0]*(1.0-a)+vars[1]*a;
    }
    else if (stat[0]) { /*按时间的最近邻推断 nearest-neighbour extrapolation by time */
        *delay=dels[0];
        *var  =vars[0];
    }
    else {
