...
#define NMAX        10              /*多项式插值的阶数 order of polynomial interpolation */
                    ...
                    double *bfact, char *tsys)
{
    ...
    for (i=0;i<22;i++) {
        if (!fgets(buff,sizeof(buff),fp)) break;//当读取 (n-1) 个字符时，或者读取到换行符时，或者到达文件末尾时，它会停止
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        for (i=pred_o=pred_c=v=0;i<n&&fgets(buff,sizeof(buff),fp);i++) {
            ...
            for (j=0;j<4;j++) {
                ...
                /*读取预测值选项  read option for predicted value */
...
extern void readsp3(const char *file, nav_t *nav, int opt)
{
    ...
    /*结合精确的星历 combine precise ephemeris */
...
static int readdcbf(const char *file, nav_t *nav, const sta_t *sta)
{
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        if      (strstr(buff,"DIFFERENTIAL (P1-P2) CODE BIASES")) type=1;//微分(P1-P2)码偏差
...
/* read dcb parameters ---------------------------------------------------------
* 读取差分代码偏置（dcb）参数 read differential code bias (dcb) parameters
...
*读取卫星小数周期偏差(dcb)参数 read satellite fractional cycle bias (dcb) parameters
...
/* 内维尔算法多项式插值 polynomial interpolation by Neville's algorithm ---------------------------*/
                   ...
                   double *dts, double *vare, double *varc)
{
    ...
    /*轨道多项式插值 polynomial interpolation for orbit */
    i=index-(NMAX+1)/2;
    if (i<0) i=0; 
    ...
    for (j=0;j<=NMAX;j++) {
        ...
        p[1][j]=pos[1];
#else
        /* 旋转的相关性 correciton for earh rotation ver.2.4.0 */
        sinl=sin(OMGE*t[j]);
        cosl=cos(OMGE*t[j]);
                      ...
                      double *dant)
{
    ...
    /* ecef中的太阳位置 sun position in ecef */
    sunmoonpos(gpst2utc(time),erpv,rsun,NULL,&gmst);
    
    /*卫星固定坐标的单位向量  unit vectors of satellite fixed coordinates */
                    ...
                    double *rs, double *dts, double *var)
{
    ...
    /*卫星天线偏移校正 satellite antenna offset correction */
    if (opt) {
    ...
    /* 相对论效应校正 relativistic effect correction */
    if (dtss[0]!=0.0) {
