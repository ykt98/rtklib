/*------------------------------------------------------------------------------
* pntpos.c :标准定位  standard positioning
*
...
#define NX          (4+3)       /* # 估计参数 of estimated parameters */

#define MAXITR      10          /* 点pos的最大迭代次数 max number of iteration for point pos */
#define ERR_ION     5.0         /* ionospheric delay std (m) */
#define ERR_TROP    3.0         /* tropspheric delay std (m) */
#define ERR_SAAS    0.3         /*模型误差标准 saastamoinen model error std (m) */
#define ERR_BRDCI   0.5         /* 广播电离层模型误差因子 broadcast iono model error factor */
#define ERR_CBIAS   0.3         /*code bias error std (m) */
#define REL_HUMI    0.7         /*saastamoinen模型的相对湿度 relative humidity for saastamoinen model */

/* 伪距测量误差方差 pseudorange measurement error variance ------------------------------------*/
...
/*带码偏校正的伪距  pseudorange with code bias correction -------------------------------------*/
                     ...
                     int iter, const prcopt_t *opt, double *var)
{
    ...
    *var=0.0;
    ...
    if (opt->ionoopt==IONOOPT_IFLC) { /* 双频 dual-frequency */
        
        if (P1==0.0||P2==0.0) return 0.0;
        if (obs->code[i]==CODE_L1C) P1+=P1_C1; /* C1->P1 */
        if (obs->code[j]==CODE_L2C) P2+=P2_C2; /* C2->P2 */
        
        /* 无离子组合 iono-free combination */
...
*          int    ionoopt   I   电离层校正选项 ionospheric correction option (IONOOPT_???)
                   ...
                   double *resp, int *ns)
{
    ...
    for (i=*ns=0;i<n&&i<MAXOBS;i++) {
        ...
        /*拒绝重复的观察数据  reject duplicated observation data */
        ...
        /* 几何距离/方位角/仰角 geometric distance/azimuth/elevation angle */
        if ((r=geodist(rs+i*6,rr,e))<=0.0||
            satazel(pos,e,azel+i*2)<opt->elmin) continue;
        
        /*伪距与代码偏差校正 psudorange with code bias correction */
        ...
        /* 对流层修正 tropospheric corrections */
        if (!tropcorr(obs[i].time,nav,pos,azel+i*2,
                      iter>0?opt->tropopt:TROPOPT_SAAS,&dtrp,&vtrp)) {
            continue;
        }
        /*伪距残差 pseudorange residual */
        v[nv]=P-(r+dtr-CLIGHT*dts[i*2]+dion+dtrp);
        
        /*设计矩阵  design matrix */
        for (j=0;j<NX;j++) H[j+nv*NX]=j<3?-e[j]:(j==3?1.0:0.0);
        
        /*时间系统和接收机偏置偏移校正 time system and receiver bias offset correction */
    ...
    /* 避免秩亏的约束 constraint to avoid rank-deficient */
    for (i=0;i<4;i++) {
        if (mask[i]) continue;
        v[nv]=0.0;
                  ...
                  char *msg)
{
    ...
    /*残差的卡方检验  chi-square validation of residuals */
    vv=dot(v,v,nv);
    ...
    dops(ns,azels,opt->elmin,dop);//计算dgop
...
/*估计接收器位置  estimate receiver position ------------------------------------------------*/
                  ...
                  double *resp, char *msg)
{
    ...
    for (i=0;i<MAXITR;i++) {
        ...
        if (norm(dx,NX)<1E-4) {
            ...
            sol->dtr[0]=x[3]/CLIGHT; /* 接收器时钟偏置（秒） receiver clock bias (s) */
            sol->dtr[1]=x[4]/CLIGHT; /*glo-gps时间偏移（秒） gl【o-gps time offset (s) */
            ...
            sol->ns=(unsigned char)ns;//可用卫星
            sol->age=sol->ratio=0.0;
            
            /* 验证解决方案validate solution */
...
/* 故障检测与排除 raim fde (failure detection and exclution) -------------------------------*/
                  ...
                  const double *azel, const int *vsat, double *v, double *H)
{
    ...
    for (i=0;i<n&&i<MAXOBS;i++) {
        ...
        /* ecef中的视线矢量line-of-sight vector in ecef */
        ...
        /* ecef中相对于接收器的卫星速度 satellite velocity relative to receiver in ecef */
        for (j=0;j<3;j++) vs[j]=rs[j+3+i*6]-x[j];
        
        /* 带地球自转校正的距离变化率 range rate with earth rotation correction */
        rate=dot(vs,e,3)+OMGE/CLIGHT*(rs[4+i*6]*rr[0]+rs[1+i*6]*x[0]-
                                      rs[3+i*6]*rr[1]-rs[  i*6]*x[1]);
        
        /*多普勒剩余 doppler residual */
...
/*估计接收器速度 estimate receiver velocity ------------------------------------------------*/
...
* 通过伪距和多普勒观测值的单点定位来计算接收器位置，速度，时钟偏差
* args   : obsd_t *obs      I   observation data
*          int    n         I   number of observation data
*          nav_t  *nav      I   navigation data
*          prcopt_t *opt    I   processing options
*          sol_t  *sol      IO  solution
*          double *azel     IO 方位角/仰角（rad）（NULL：无输出）  azimuth/elevation angle (rad) (NULL: no output)
...
*          and receiver bias)
* 假设sbas-gps，galileo-gps，qzss-gps，指南针-gps时间偏移和接收器偏差可忽略不计（仅涉及glonass-gps时间偏移和接收器偏差）
*-----------------------------------------------------------------------------*/
                  ...
                  char *msg)
{
    ...
    if (opt_.mode!=PMODE_SINGLE) { /*精确定位  for precise positioning */
#if 0
    ...
    /* 卫星位置，速度和时钟 satellite positons, velocities and clocks */
    satposs(sol->time,obs,n,nav,opt_.sateph,rs,dts,var,svh);//rs[0-2],rs[3-5]
    
    /* 用伪距估计接收器位置 estimate receiver position with pseudorange */
    ...
    /*用多普勒估计接收机速度  estimate receiver velocity with doppler */
