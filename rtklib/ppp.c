...
#define THRES_REJECT 4.0            /* posfit-res的剔除阈值(sigma) reject threshold of posfit-res (sigma) */
    ...
    const double* rs, double* exs, double* eys)
{
    ...
    /*卫星偏航角 yaw-angle of satellite */
    if (!yaw_angle(sat, type, opt, beta, mu, &yaw)) return 0;

    /*卫星固定x，y矢量 satellite fixed x,y-vector */
    cross3(en, es, ex);
    cosy = cos(yaw);
    siny = sin(yaw);
    for (i = 0; i < 3; i++) {
    ...
    const double* rs, const double* rr, double* phw)
{
    ...
    /*卫星偏航姿态模型 satellite yaw attitude model */
    if (!sat_yaw(time, sat, type, opt, rs, exs, eys)) return 0;

    /*卫星到接收机单位矢量 unit vector satellite to receiver */
    for (i = 0; i < 3; i++) r[i] = rr[i] - rs[i];
    if (!normv3(r, ek)) return 0;


    /*接收天线的单位向量 unit vectors of receiver antenna */
    ecef2pos(rr, pos);
    xyz2enu(pos, E);
    exr[0] = E[1]; exr[1] = E[4]; exr[2] = E[7]; /* x = north */
    eyr[0] = -E[0]; eyr[1] = -E[3]; eyr[2] = -E[6]; /* y = west  */

    /*相位饱和效应 phase windup effect */
    cross3(ek, eys, eks);
    cross3(ek, eyr, ekr);
    for (i = 0; i < 3; i++) {
...
/* 初始化状态和协方差 initialize state and covariance --------------------------- ----------------*/
    ...
    double* Lc, double* Pc)
{
    ...
    for (i = 0; i < NFREQ; i++) {
        ...
        /*天线相位中心和相位饱和校正 antenna phase center and phase windup correction */
...
static void udpos_ppp(rtk_t* rtk)
{
    ...
    /*初始化第一个纪元的位置 initialize position for first epoch */
    if (norm(rtk->x, 3) <= 0.0) {
    ...
    /*生成有效的状态索引 generate valid state index */
    ix = imat(rtk->nx, 1);
    ...
    /* 位置/速度/加速度的状态转换 state transition of position/velocity/acceleration */
    ...
    /*仅加速时增加的过程噪声 process noise added to only acceleration */
...
static void udclk_ppp(rtk_t* rtk)
{
    ...
    /*初始化时钟的每个时期(白噪声) initialize every epoch for clock (white noise) */
    for (i = 0; i < NSYS; i++) {
        if (rtk->opt.sateph == EPHOPT_PREC) {
            /* prec星历的时间基于gpsttime of prec ephemeris is based gpst */
            /* 负选择接收器系统间偏差 negelect receiver inter-system bias  */
            dtr = rtk->sol.dtr[0];
        }
        else {
...
static void udbias_ppp(rtk_t* rtk, const obsd_t* obs, int n, const nav_t* nav)
{
    ...
    /*处理日边界时钟跳变 handle day-boundary clock jump */
    if (rtk->opt.posopt[5]) {
    ...
    for (f = 0; f < NF(&rtk->opt); f++) {
        ...
        /*如果obs中断计数器过期，重置相位偏置 reset phase-bias if expire obs outage counter */
        for (i = 0; i < MAXSAT; i++) {
        ...
        /*纠正相位码跳变以确保相位码一致性 correct phase-code jump to ensure phase-code coherency */
...
static void udstate_ppp(rtk_t* rtk, const obsd_t* obs, int n, const nav_t* nav)
{
    ...
    /* 位置更新 temporal update of position */
    udpos_ppp(rtk);

    /*时钟 temporal update of clock */
    udclk_ppp(rtk);

    /* 对流层参数temporal update of tropospheric parameters */
    if (rtk->opt.tropopt == TROPOPT_EST || rtk->opt.tropopt == TROPOPT_ESTG) {
        udtrop_ppp(rtk);
    }
    /*电离层参数 temporal update of ionospheric parameters */
    if (rtk->opt.ionoopt == IONOOPT_EST) {
        udiono_ppp(rtk, obs, n, nav);
    }
    /*L5-接收器-dcb参数 temporal update of L5-receiver-dcb parameters */
    if (rtk->opt.nf >= 3) {
        uddcb_ppp(rtk);
    }
    /*相位偏差 temporal update of phase-bias */
...
/*相位和代码残差 phase and code residuals --------------------------------------------------*/
    ...
    double* azel)
{
    ...
    for (i = 0; i < n && i < MAXOBS; i++) {
        ...
        /* 对流层和电离层模型tropospheric and ionospheric model */
        ...
        for (j = 0; j < 2 * NF(opt); j++) {
            ...
            /*通过预拟合残差剔除卫星 reject satellite by pre-fit residuals */
            ...
            /*记录大的拟合后残差 record large post-fit residuals */
    ...
    /*剔除具有较大和最大拟合后残差的卫星 reject satellite with large and max post-fit residual */
    if (post && ne > 0) {
    ...
    /*局部校正的约束 constraint to local correction */
